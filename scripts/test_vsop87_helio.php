<?php
/**
 * Test VSOP87 heliocentric coordinates for all outer planets
 */
...
foreach ($planets as $name => $planet) {
    ...
    $ret_vsop = swe_calc($jd, $planet, Constants::SEFLG_VSOP87 | Constants::SEFLG_HELCTR, $xx_vsop, $serr);
    $ret_swieph = swe_calc($jd, $planet, Constants::SEFLG_SWIEPH | Constants::SEFLG_HELCTR, $xx_swieph, $serr);
...
echo "=== VSOP87 Heliocentric Accuracy (vs SWIEPH) ===\n";
...
// Check if all planets pass heliocentric accuracy test (< 36 arcsec for angles, < 100km for distance)
$max_arcsec = 36.0;
$max_dist_km = 100000.0;
$all_pass = true;
...
foreach ($results as $name => $errors) {
    ...
    if ($errors['dist_km'] > $max_dist_km) {
        echo "\n✗ $name FAILED distance accuracy (> 100,000 km)\n";
        $all_pass = false;
    }
}

if ($all_pass) {
    echo "\n✓ All planets PASSED heliocentric accuracy test\n";
    exit(0);
} else {
    exit(1);
}
