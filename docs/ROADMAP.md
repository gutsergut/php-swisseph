...
1. **Сидерика**: довести "True" режимы аянамши (требует `swe_fixstar` или альтернативу)
2. **Сидерика**: применение опций `SE_SIDBIT_*` в расчётах
3. **Узлы/апсиды**: полная реализация координатных преобразований
4. **Rise/Set/Transit**: расширенная поддержка флагов и twilight
5. **Тесты**: больше parity-тестов со swetest для всех функций
6. **Документация**: примеры использования, таблица точности, FAQ
