...
- **Планеты (кроме Луны)**: ещё используют упрощённые модели Meeus (VSOP87/JPL интеграция запланирована)
- **Полярные широты**: Placidus/Koch могут возвращать ошибку (SE_ERR)
- **Rise/Set методы 2-5**: требуют улучшения точности горизонта и рефракции
- **Osculating узлы/апсиды**: в работе

### Точность Луны (достижение Q4 2025)
- Геоцентрические экваториальные координаты: RA ошибка ≈ 0.000"; Dec ошибка ≈ 0.001"
- Эклиптические: долговременная погрешность (lon) ~4.7" на тестовом дне (при использовании ускорения без VSOP87); lat ≈1.8"
- Топоцентрический параллакс: отношение смещений RA/Dec (PHP / swetest) = 1.0014 / 1.0000 (в пределах 0.14%)
- Реализация включает:
  - Полная годичная аберрация (Lorentz формула)
  - Двойная нутация: матрица + матрица скорости (`NUT_SPEED_INTV = 0.0001` суток)
  - Frame bias (ICRS→J2000)
  - Централизованный источник обликвитета через `EpsilonData` (mean obliquity of date, J2000)
  - Кэш нутации и матриц в `SwedState::ensureNutation()`

### Новые тесты точности
- `MoonAberrationAccuracyTest.php` — субарксекундная проверка RA/Dec
- `MoonTopoParallaxTest.php` — проверка относительного топо параллакса (±0.2%)
- `diagnose_moon_steps.php` — многостраничная диагностика цепочки преобразований
